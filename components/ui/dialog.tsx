  ...
  return (
    <FullWindowOverlay>
      ...
      >
        ...
        >
          {/* Transparent full-screen pressable that closes the dialog */}
    ...
    <DialogPortal hostName={portalHost}>
      <DialogOverlay onClose={() => onOpenChange?.(false)}>
        <DialogPrimitive.Content
          className={cn(
            "bg-background border-border z-50 mx-auto flex w-full max-w-[calc(100%-2rem)] flex-col gap-4 rounded-[22px] border p-6 shadow-lg shadow-black/5 sm:max-w-lg",
            Platform.select({
...
function DialogTitle({
  ...
  return (
    <DialogPrimitive.Title
      className={cn(
        "text-foreground text-xl font-inter-semibold leading-snug tracking-tight",
        className,
      )}
      {...props}
    />
...
function DialogDescription({
  ...
  return (
    <DialogPrimitive.Description
      className={cn(
        "text-base font-inter-medium leading-relaxed text-muted-foreground",
        className,
      )}
      {...props}
    />
